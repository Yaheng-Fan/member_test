...
# 本程序的数据扩增为图像输入网络前扩增,不是resize为128之前扩增

exp_name = "exp2"
root_path = r'../scanners_70/'
#root_path = r'../scanner1_test/'
root_result = r'../results/'+exp_name+'/test_TTA'
model_dir = r"../models/"+exp_name + '/1/'
...
parser.add_argument('--start_epoch', default=5, type=int, help='manual epoch number (useful on restarts)')
...
# def weight_init_normals(module):
#     for m in module.modules():
#         if isinstance(m, (nn.Conv3d, nn.Linear)):
#             nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
#             # fan_out 会出现loss不变的问题 # 还是会出现问题
#
# # 网络权重初始化
# models.apply(weight_init_normals)
    ...
    if train == True and three_d == False:  # 2D训练
        ...
        for i in range(len(file)):  # 患者
            ...
            for ii in range(len(patient_slice)):  # 每个患者有病灶层数
                ...
                grade_slice.append(grade)
                slice_ct_seg = list(
                    zip(ct_slice, seg_gt_slice, grade_slice))  # 每一层ct,seg,grade合并到同一个list,元组结果按矩阵最短的算，所以grade弄成列表
...
def change_to_3d_data(data):
    ...
    for i in range(len(data)):
        ...
        ct_3d, seg_3d, grade_3d = train_data_normalization(ct_3d, seg_3d, grade_3d,
                                                           extra_expend_voxel=expend_voxel_size,
...
def train_net(data, model, criterion, optimizer, epoch, lr_cur, fold):
    ...
    for i, b in enumerate(dataloader):  # 对图像进行训练
        ...
        print('Fold:{}, Epoch:{}, batch:{}/{}, Lr:{}, loss:{}, auc:{}, acc:{}'.format(fold, epoch, i + 1,
                                                                                      len(data) // args.batch_size + 1,
                                                                                      lr_cur, loss.item(), auc_train,
                                                                                      acc_train))
    ...
    print('Epoch:{} Finished, Lr:{}, Loss:{}, Auc:{}, Acc:{}'.format(epoch, lr_cur, np.mean(loss_epoch), all_train_auc,
                                                                     all_train_acc))
    ...
    if val == True:
        ...
        for ind, patient in enumerate(data):  # 读每个人的所有3d块
            ...
            for i, b in enumerate(patient):  # 读每一3d块
                ...
                print('Fold:{}, Epoch:{}, 【patient_num:{}】, num:{}/{}, loss:{}， probability:{}\t'.format(fold, epoch,
                                                                                                         val_patient[
                                                                                                             ind],
                                                                                                         i + 1,
                                                                                                         len(patient),
            ...
            print('Fold:{}, Epoch:{}, 【patient_num:{}】, Loss:{}, probability:{}, label:{}\t'.format(fold, epoch,
                                                                                                    val_patient[ind],
                                                                                                    np.mean(loss_val),
                                                                                                    patient_probability,
                                                                                                    int(grades_label_np[
        ...
        print('Fold:{}, VALIDATION Finished, loss:{}, Auc:{}, Acc:{}'.format(fold, np.mean(loss_val_all), all_val_auc,
                                                                             all_val_acc))
        print('------------------------------------------------------------')
...
def test_net(model, data, epoch, fold, TTA=False, TTA_times=None):
    ...
    if TTA == False:
        ...
        for ind, patient in enumerate(data):  # 读每个人的所有3d块
            ...
            for i, b in enumerate(patient):  # 读每一3d块
                ...
                if i == 0:
                    ...
                    outputs_all = np.concatenate((outputs_all, outputs_np), axis=0)
                    grades_label_all = np.concatenate((grades_label_all, grades_label_np), axis=0)
                #print('Fold:{}, Epoch:{}, 【patient_num:{}】, num:{}/{}, loss:{}， probability:{}\t'.format(fold, epoch, test_patient[ind], i + 1, len(patient), loss.item(), outputs_np[0][0]))

            # 每个人平均概率，以及所有人的平均概率、目标类别累计
            patient_probability = np.mean(outputs_all)
            print('Fold:{}, Epoch:{}, 【patient_num:{}】, Loss:{}, probability:{}, label:{}\t'.format(fold, epoch, test_patient[ind], np.mean(loss_val), patient_probability, int(grades_label_np[0][0])))
            print('------------------------------------------------------------')
        ...
        possibility_save(epoch, test_patient, [all_probability, all_grade, all_val_auc], fold)
        ...
        for ind, patient in enumerate(data):  # 读每个人的所有3d块
            ...
            for i, b in enumerate(patient):  # 读每一3d块
                ...
                for tta_times in range(TTA_times):
                    imgs_aug = TTA_transform(imgs[0])
                    imgs_aug = imgs_aug[np.newaxis, :]
                    imgs_aug = torch.from_numpy(imgs_aug).unsqueeze(
                        1)  # 转成tensor；并，对指定位置，unsqueeze是对数据维度进行扩充，squeeze是对数据维度进行压缩
                    grades_label = torch.from_numpy(grade_input.astype(np.float32))  # .unsqueeze(1)
                    # 数据用gpu跑
                    if args.gpu:
        ...
        print('Fold:{}, Test TTA Finished, loss:{}, Auc:{}, Acc:{}'.format(fold, np.mean(loss_val_all), all_val_auc,
                                                                           all_val_acc))
        print('------------------------------------------------------------')
...
if __name__ == "__main__":
    ...
    fold = 1
    test_patient = os.listdir(root_path)

    print('test_patient:', test_patient)
    print(len(test_patient))

    # load data --------------------------------------------------------------------------------------------------------
    test_data, test_num_list_slice = get_h5_perpatient_perslice(root_path, test_patient, train=True, three_d=True)

    # get 3d block -----------------------------------------------------------------------------------------------------
    test_block_all_per_patient, test_block_all = my_get_3d_block(test_data, args.block_z_slice, args.block_z_sample_slice)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        epoch_file = os.path.join('epoch_' + str(epoch) + '_model.pkl')
        epoch = np.int32(epoch_file.split('_')[1])
        try:
            model.load_state_dict(torch.load(os.path.join(model_path, epoch_file)))  # 加载模型
        except:
            continue
        all_epoch_list.append(epoch)

        # test for one epoch_______________________________________________________________________________________
        loss_test_all, all_probability_test, all_grade_test, auc_test, acc_test = test_net(model, test_block_all_per_patient, epoch, fold, TTA=False, TTA_times=10)  # models, data, epoch, fold, TTA=False, TTA_times=None

        #loss_test_epoch_mean.append(np.mean(loss_test_all))
        #all_test_auc_list.append(auc_test)
        #all_test_acc_list.append(acc_test)
        #LOSS_excel_save(os.path.join(root_result, "test_loss_fold" + str(fold) + ".xls"), all_epoch_list, loss_test_epoch_mean)
        #AUC_excel_save(os.path.join(root_result, "test_auc_fold" + str(fold) + ".xls"), all_epoch_list, all_test_auc_list)

        # draw loss_____________________________________________________________________________________________
        ## train loss
        #x = all_epoch_list
        #y1 = loss_test_epoch_mean
        #plt.plot(x, y1, label='test loss')
        #plt.xlabel("epoch")
        #plt.ylabel("loss")
        #plt.title("Loss")
        #plt.savefig(os.path.join(root_result, "test_loss_fold" + str(fold) + ".jpg"))
        #plt.close()
